...
def align_bowtie2(output, no_clean, reference, read1, read2):
    ...
    if read2 is None:
        return func_align_bowtie2(output, no_clean, reference, read1)
    else:
        return func_align_bowtie2(output, no_clean, reference, read1, read2)
...
def align_bwa(output, nthreads, no_clean, reference, read1, read2):
    ...
    if read2 is None:
        return func_align_bwa(output, nthreads, no_clean, reference, read1)
    else:
        return func_align_bwa(output, nthreads, no_clean, reference, read1, read2)
...
def align_tmap(output, reference, read1, read2):
    ...
    if read2 is None:
        return func_align_tmap(output, reference, read1)
    else:
        return func_align_tmap(output, reference, read1, read2)
...
@click.option('--output', '-o', default='processed.bam', help='Name of the final output file.')
@click.option('--add-known-indels', '-i', default='', help='Additional files containing known indel information.',
              multiple=True)
@click.option('--readgroup-info', default='ID:42,PU:123,PL:ILLUMINA,SM:sample1,LB:1',
              type=str, help='Add read group information  to the sample, which MUST '
...
@click.argument('read2', required=False, type=click.Path(exists=True))
@click.pass_context
def run(ctx, output, add_known_indels, readgroup_info, aligner, caller, known_indels, known_snps, reference, read1, read2):
    ctx.invoke(align_bwa, reference, read1, read2, output='bwa.out', no_clean=False)

    align_out = aligner+'_out.bam'
    process_out = aligner+'_out.bam.processed.bam'

    align_args = ['vcaller', 'align', aligner, 'reference', 'read1', 'read2']
    process_args = ['vcaller', 'process', '-o', process_out, '--readgroup-info', readgroup_info,
                    '--add-known-indels', add_known_indels, known_indels, known_snps, reference, align_out]
    call_args = ['vcaller', 'call', caller, '-o', output, reference, process_out]

    print(align_args)
    run(align_args)
    run(process_args)
    run(call_args)
    cleanup([align_out, process_out])


    # t = pipes.Template()
    # t.append('vcaller align %s %s %s %s' % (aligner, reference, read1, read2), '--')
    # t.append('vcaller call %s %s %s' % (caller, reference, '-'), '--')
    # t.append('vcaller process -o %s --readgroup-info %s --add-known-indels %s %s %s %s %s' %
    #          (output, readgroup_info, add_known_indels, known_indels, known_snps, reference, '-'), '--')
    # f = t.open('pipefile', 'w')